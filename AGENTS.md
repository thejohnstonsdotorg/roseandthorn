...
- **Generative imagery is procedural-first.** `lib/proceduralArt.ts` runs synchronously (<100ms) and is always the default path and silent-fallback safety net. There are two opt-in AI backends available in Settings: **MediaPipe** (on-device SD 1.5, ~3–8 s, strictly offline, 1.9 GB download) and **Cloud** (FLUX.1 schnell via fal.ai / Fireworks / Together AI / Replicate / WaveSpeed, <2 s, requires internet + family's own BYO API key). The active backend is selected in `settingsStore.aiBackend` and dispatched through `lib/imageGen.ts`. Neither AI backend must ever block screen transitions — generation is always fire-and-forget with procedural as the silent fallback.
